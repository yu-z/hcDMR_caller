...
for infile in infiles:
    ...
    for line in fin:
        ...
        if not Steve_filter_flag:
            ...
            while index >= 0:
                if depthcr[pos+index] < 65535: #Don't understand this, is this a cap on coverage?
...
#modified by Jixian
filename = options.outfile + '.gz'
fout = gzip.open(filename, 'wb')

logfname = options.outfile + '.BSMAP_log'
mlog = open(logfname, 'w')
...
fout.close()
disp('done.')
if nc > 0: mlog.write ('total %d valid mappings, %d reads tossed by Steve filter, %d covered cytosines, average coverage: %.2f fold.' % (nmap, Steve_filter_total,nc, float(nd)/nc))
