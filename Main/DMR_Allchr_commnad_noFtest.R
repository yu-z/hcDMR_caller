...
CG.pvalue.df=data.frame(CG.amd.df,CG.p.value)
CHG.pvalue.df=data.frame(CHG.amd.df,CHG.p.value)
CHH.pvalue.df=data.frame(CHH.amd.df,CHH.p.value)

#pvalue < 0.01
CG.DMR.mutant.control=subset(CG.pvalue.df,CG.pvalue.df[,8]<=0.01)
#dim(CG.DMR.mutant.control)
#head(CG.DMR.mutant.control)
CHG.DMR.mutant.control=subset(CHG.pvalue.df,CHG.pvalue.df[,8]<=0.01)
#dim(CHG.DMR.mutant.control)
CHH.DMR.mutant.control=subset(CHH.pvalue.df,CHH.pvalue.df[,8]<=0.01)
#dim(CHH.DMR.mutant.control)

dir.create(file.path(args[4]))

write.table(CG.pvalue.df,file=paste(args[4], "CG.DMR.", args[1], "_VS_", args[2], ".txt", sep=""),sep="\t",row.names=F)
write.table(CHG.pvalue.df,file=paste(args[4], "CHG.DMR.", args[1], "_VS_", args[2], ".txt", sep=""),sep="\t",row.names=F)
write.table(CHH.pvalue.df,file=paste(args[4], "CHH.DMR.", args[1], "_VS_", args[2], ".txt", sep=""),sep="\t",row.names=F)
